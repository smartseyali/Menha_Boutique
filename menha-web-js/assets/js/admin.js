...
// ── STATE ─────────────────────────────────────────────────
let allProducts = [], allCategories = [], allOrders = [], allUsers = [], allBanners = [], allTariffs = [], allStates = [];
...
function renderVariants() {
    ...
    container.innerHTML = productVariants.map((v, i) => `
        <div style="display:flex; gap:10px; align-items:center; background:#fff; padding:10px; border:1px solid var(--color-border); border-radius:6px;">
            <div style="flex:1">
...
function renderProducts(list) {
    ...
    tbody.innerHTML = list.map(p => `
        <tr>
            ...
            <td><span class="badge-${p.status === 'In Stock' ? 'active' : 'inactive'}">${p.status}</span></td>
            <td>
...
async function saveProduct(e) {
    ...
    try {
        ...
        if (extra_images.length > 0 && resultProdId) {
            try { await Supabase.from('product_images').eq('product_id', resultProdId).delete(); } catch(e){} // CLEAR existing extra images gracefully since new layout provides absolute mapping!
        ...
        if (resultProdId) {
            try { await Supabase.from('product_attributes').eq('product_id', resultProdId).delete(); } catch(e){}
...
async function loadOrders() {
    ...
    try {
        ...
        allOrders = await Supabase.from('orders')
            .select('*, order_items(*, product:products(title, primary_image)), address:addresses(*)')
...
async function viewOrderDetails(id) {
    ...
    document.getElementById('odm-courier').innerHTML = `
        ...
        <strong>Courier ID:</strong> ${order.courier_id || 'N/A'}
    `;
...
function exportOrdersToExcel() {
    ...
    let csv = "Order Number,Date,Email,Customer Name,Phone,Address,City,State,Zip,Total,Status,Payment Method\n";
    filtered.forEach(o => {
        const addr = o.address || {};
        const fullName = `"${(addr.first_name || '')} ${(addr.last_name || '')}"`;
        const fullAddr = `"${(addr.address_line1 || '')} ${(addr.address_line2 || '')}"`.replace(/\n/g, ' ');
        const date = new Date(o.created_at).toLocaleDateString();
        
        csv += `${o.order_number || o.id.substring(0,8)},${date},${o.email || ''},${fullName},${addr.phone_number || ''},${fullAddr},${addr.city || ''},${addr.state || ''},${addr.zip_code || ''},${o.total_price},${o.status},${o.payment_method || 'COD'}\n`;
    });
